...
//     trapdoor *trap;
//     if(!(trap = (trapdoor *)calloc(1,sizeof(trapdoor)))){
//         fprintf(stderr,"[*] Error in initializing bloom filter.\n");
//         exit(-1);
//     }

//     if((trap->KPlusOneTrapdoor = malloc((NUMBEROFKEYS)*sizeof(oneoftrapdoor *)))==NULL)
//         printf("Data allocation failed\n");
//     for(int i = 0; i < NUMBEROFKEYS; i++){
//         if((trap->KPlusOneTrapdoor[i] =(oneoftrapdoor *)malloc(sizeof(oneoftrapdoor)))==NULL){
//             printf("Data allocation failed\n");
//             free(trap->KPlusOneTrapdoor);
//             exit(-1);
//         }
//     }
//     return trap;
...
//     if(trap == NULL){
//         fprintf(stderr, "error in free a null pointer\n");
//         exit(-1);
//     }
//     for(int i = 0; i < NUMBEROFKEYS;i++)
//         free(trap->KPlusOneTrapdoor[i]);
//     free(trap->KPlusOneTrapdoor);
//     free(trap);
// }
...
bool is_trapdoor_in_bloom(DOUBLENODEBLOOMFILTER *bf, trapdoor *trap){
    unsigned char count = 0;
    for(int i = 0; i < NUMBEROFKEYS; i++){
        unsigned short bit_pos;
        unsigned long shor_pos;
        unsigned long bit_postmp = trap->KPlusOneTrapdoor[i]->onetrapdoor;
        shor_pos = bit_postmp >> 3;
        bit_pos = bit_postmp &0x7;
        char chosen_pos = getChosenCellLocationV2(trap->KPlusOneTrapdoor[i]->hashvalueofK_Plus_One,bf->rb);
        if(chosen_pos == 0)
...
bool getChosenCellLocationV2(BYTE hashvalueOFkplusone[],unsigned int rb){
    BYTE hashvalueOFkplusonetmp[SHA256_BLOCK_SIZE];
    for(int i = 0; i < 28;i++)
    ...
    BYTE sha1hashvalue[SHA1_BLOCK_SIZE];
